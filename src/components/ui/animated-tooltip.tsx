...
export const AnimatedTooltip = ({ items }: { items: TooltipItem[] }) => {
  ...
  return (
    <div>
      ...
      <div className="h-[30vh] px-20 flex items-center justify-center w-full text-center">
        &ldquo;{items[activeIndex]?.body}&rdquo;
      </div>
      <p className="text-center">@{items[activeIndex]?.name}</p>
      <p className="text-center  text-gray-500">{items[activeIndex]?.designation}</p>
    </div>
  );
};
